...
from sequenceNet import NeuralNet
...
class StackedBidirectional(NeuralNet):
    def __init__(self, review_summary_file, checkpointer, num_layers, attention=True):
        """
        A  Stacked Bidirectional ([Forward + Backward direction], Many Layers) Seq2Seq Encoder-Decoder model
        review_summary_file: The file containing the (review, summary) pair in CSV format
                review is the full text, summary is the given summary
        checkpointer: The checkpoint handling object [Object]
        num_layers: Number of bi-directional LSTM layers
        attention: If attention mechanism is to be implemented, this boolean variable is True. 
                Here we save the model for non-attention mechanism. The Default is True
        """
    ...
    def _split_train_tst(self):
        ...
        Create the X_trn, X_tst, and Y_trn and Y_tst for both forward and backward
        Only the reviews are changed. The given summaries are fixed.
        """
    ...
    def _load_data(self):
        ...
        If the data has been checkpointed before this load, then load the checkpointed data instead
        """
        self.mapper = Mapper()
    ...
    def _load_data_graph(self):
        """
        Loads the data graph consisting of bi-directional review placeholders, summary(Target tip summary)
                               placeholders and the weights of the hidden layer of the Seq2Seq model.
        """
        # input
        with tf.variable_scope("train_test", reuse=True) as scope:
            # Encoder input
            ...
            # Decoder input
            # Prepend some "GO" token and drop the final token of the encoder input
            self.dec_inp = ([tf.zeros_like(self.labels[0], dtype=np.int32, name="GO")] + self.labels[:-1])

    def _load_model(self):
        """
        Create model for encoder and decoder respectively
        Both use neural machine translation models
        """
        # bi-LSTM:
        # Initial memory value for recurrence.
        self.prev_mem = tf.zeros((self.train_batch_size, self.memory_dim))

        # choose LSTM cell through lstm_stacked_bidirectional.py
        with tf.variable_scope("forward") as scope:
            fw_single_cell = self.get_cell()
            # Multi-layers of forward LSTM cells
            self.forward_cell = tf.contrib.rnn.MultiRNNCell([fw_single_cell] * self.num_layers)
        with tf.variable_scope("backward") as scope:
            bw_single_cell = self.get_cell()
            # Multi-layers of backward LSTM cells
            self.backward_cell = tf.contrib.rnn.MultiRNNCell([bw_single_cell] * self.num_layers)

        # The following part have something wrong
        # Because py2 and py3, and tensorflow 1.2 and tensorflow 1.5 have different API
        # I need more time to get it correct, I will do this part on spring break referring to the tutorial on tensorflow
        # Tutorial of this part: tf.conrib.seq2seq Overview
        """
        # Decoder models
        # Bahdanau attention model or basic model
        # Encoder divided in bi-directional layers
        if not self.attention:
            # Only take the outputs, the states are ignored        
            with tf.variable_scope("forward"):
                       
                self.dec_outputs_fwd, _ = tf.contrib.seq2seq.BasicDecoder(
                                self.enc_inp_fwd, self.dec_inp, self.forward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length)
            with tf.variable_scope("forward", reuse=True):
                self.dec_outputs_fwd_tst, _ = tf.contrib.seq2seq.BasicDecoder(
                                self.enc_inp_fwd, self.dec_inp, self.forward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length, feed_previous=True)

            with tf.variable_scope("backward"):
                self.dec_outputs_bwd, _ = tf.contrib.seq2seq.BasicDecoder(
                                self.enc_inp_bwd, self.dec_inp, self.backward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length)

            with tf.variable_scope("backward", reuse=True):
                self.dec_outputs_bwd_tst, _ = tf.contrib.seq2seq.BasicDecoder(
                                self.enc_inp_bwd, self.dec_inp, self.backward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length, feed_previous=True)

        else:
            with tf.variable_scope("forward"):
                self.dec_outputs_fwd, _ =  tf.contrib.seq2seq.BahdanauAttention(
                                self.enc_inp_fwd, self.dec_inp, self.forward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length)
            with tf.variable_scope("forward", reuse=True):
                self.dec_outputs_fwd_tst, _ =  tf.contrib.seq2seq.BahdanauAttention(
                                self.enc_inp_fwd, self.dec_inp, self.forward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length, feed_previous=True)

            with tf.variable_scope("backward"):
                self.dec_outputs_bwd, _ =  tf.contrib.seq2seq.BahdanauAttention(
                                self.enc_inp_bwd, self.dec_inp, self.backward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length)

            with tf.variable_scope("backward", reuse=True):
                self.dec_outputs_bwd_tst, _ =  tf.contrib.seq2seq.BahdanauAttention(
                                self.enc_inp_bwd, self.dec_inp, self.backward_cell,
                                self.vocab_size, self.vocab_size, self.seq_length, feed_previous=True)
        """
    ...
    def _load_optimizer(self):
        """
        Define optimizer and loss function to forward and backward respectively
        """
        # Two loss functions
        with tf.variable_scope("forward"):
            self.loss_fwd = tf.contrib.seq2seq.sequence_loss(self.dec_outputs_fwd, self.labels,
        ...
        with tf.variable_scope("backward"):
            self.loss_bwd = tf.contrib.seq2seq.sequence_loss(self.dec_outputs_bwd, self.labels,
                                                        self.weights, self.vocab_size)
            # optimizer
    ...
    def _train_batch(self, review_fwd, review_bwd, summary):
        ...
        review: The input review data (X) shape[seq_length x batch_length]
        summary: The target tip data (Y) shape[seq_length x batch_length]
        """
    ...
    def _visual_validate(self, review_fwd, review_bwd, true_summary):
        ...
        review: The input review sentence
        true_summary: The true summary (target)
        """
    ...
    def generate_one_summary(self, review_fwd, review_bwd):
        ...
        Return the output Summary of the model
    ...
    def _predict_batch(self, review_fwd, review_bwd):
        ...
        review_fwd: Input review batch for forward propagation model
        review_bwd: Input review batch for backward propagation model
        """
        summary_out = []
        # Forward
